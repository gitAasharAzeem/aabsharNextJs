...
const SearchResults = ({ mosques }: SearchResultsProps) => {
    return (
                            ...
                            <div key={mosque.id} className={styles.contributeMosqueCard}>
                                <div className={styles.imageContainer}>
                                    ...
                                    {mosque.image?.featuredImage?.path ? (
                                        ...
                                        <div className={styles.mosqueImagePlaceholder}>No Image Available</div>
                                    )}
                                    <div className={styles.mosqueNameOverlay}>{mosque.name}</div>
                                </div>
                                <div className={styles.mosqueDetails}>
                                    <p className={styles.mosqueDetailsText}>Area: {mosque.area}</p>
                                    <p className={styles.mosqueDetailsText}>City: {mosque.city}</p>
                                    <p className={styles.mosqueDetailsText}>Total Contribution: {mosque.totalContribution}</p>
                                </div>
...
// Use Server-Side Rendering to fetch mosques based on query parameters
export async function getServerSideProps(context: any) {
    const { city = '', size = '', contribution = '' } = context.query;
