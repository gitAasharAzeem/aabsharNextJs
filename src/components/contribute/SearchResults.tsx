...
import { useSearchParams } from 'next/navigation'; // Get search params using Next.js 13+ hooks
...
const SearchResults = () => {
    const searchParams = useSearchParams(); // Get search params using Next.js 13 hooks
    ...
    useEffect(() => {
        const fetchMosques = async () => {
            try {
                ...
                const params: any = { sortOption: 'ASC' }; // sortOption is mandatory

                // Only add params if values are present
                if (city) params.city = city;
                if (size) params.noOfPeople = size;
                if (contribution) params.contribution = contribution;

                // Make API request
                const response = await axios.get('https://www.admin.aabshar.net/api/v1/public/mosque/search', { params });
                setMosques(response.data.data || []); // Set response data
            } catch (error) {
        ...
        // Fetch mosques whenever there are valid query parameters
        fetchMosques();
    }, [city, size, contribution]);
